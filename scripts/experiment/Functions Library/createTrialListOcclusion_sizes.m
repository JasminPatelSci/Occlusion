function [TrialList] = createTrialListOcclusion_sizes(nFlashLocations, localizerTrials, motionTrials)
...
locTrialListS1 = repelem(flashLocs,1,localizerTrials*2/nFlashLocations)'; %for first session, double the amount of localiser flashes
locTrialList = repelem(flashLocs,1,localizerTrials/nFlashLocations)';
locTrialList = locTrialList(randperm(size(locTrialList,1)),1);

TrialList{1,1} = locTrialListS1(randperm(size(locTrialListS1,1)),1);
TrialList{1,2} = locTrialList(randperm(size(locTrialList,1)),1);
TrialList{1,3} = locTrialList(randperm(size(locTrialList,1)),1); % added 3rd session flashes
TrialList{1,4} = locTrialList(randperm(size(locTrialList,1)),1); % added 4th session flashes

%% Smooth motion block

startLoc = repelem(flashLocs,1,motionTrials/nFlashLocations)';
trialDir = repelem([-1,1], 1, motionTrials/2)';
occluderSize = repelem([0,1,2], 1, motionTrials/3)';

for session = 1:4 % 4 sessions
    ...
    occluderSize = occluderSize(randperm(size(occluderSize,1)),1);
    
    trialDir = trialDir(randperm(size(trialDir,1)),1);
      
    TrialList{2,session} = [startLoc, endLoc, trialDir, occlusionLoc, occluderSize];
